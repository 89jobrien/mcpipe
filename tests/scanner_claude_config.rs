...
fn discovered_source_roundtrip() {
    let src = DiscoveredSource {
        ...
        kind: BackendKind::McpStdio {
            command: "my-server mcp".to_string(),
        },
...
async fn scans_settings_json_mcp_servers() {
    ...
    let scanner = ClaudeConfigScanner::from_paths(vec![settings_path.to_string()], vec![]);
...
async fn scans_project_mcp_json() {
    ...
    let scanner = ClaudeConfigScanner::from_paths(vec![], vec![mcp_path.to_string()]);
    let sources = scanner.scan().await;
