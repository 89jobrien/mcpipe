...
impl McpBackend {
    ...
    async fn run_stdio_session<F, Fut>(
        &self,
        command: &str,
        f: F,
        ...
        Fut: std::future::Future<Output = Result<serde_json::Value, BackendError>>,
    {
        ...
        let prog = parts
            .next()
            .ok_or_else(|| BackendError::Transport("empty command".to_string()))?;
            ...
            match stream.next().await {
                ...
                Some(Err(e)) => {
                    return Err(BackendError::Transport(format!(
                        "SSE error waiting for endpoint: {e}"
                    )));
                }
                None => {
...
type SseStream = std::pin::Pin<
    Box<dyn futures::Stream<Item = Result<SSE, eventsource_client::Error>> + Send + Sync>,
>;
...
struct HttpSession {
    ...
    pending:
        std::sync::Arc<Mutex<std::collections::HashMap<u64, oneshot::Sender<serde_json::Value>>>>,
    next_id: Mutex<u64>,
        ...
        match &self.inner {
            McpTransport::Stdio { command } => {
                ...
                self.run_stdio_session(&command, |session| async move {
                    ...
                    serde_json::to_value(cmds).map_err(|e| BackendError::Schema(e.to_string()))
                })
                .await
                .and_then(|v| {
                    serde_json::from_value(v).map_err(|e| BackendError::Schema(e.to_string()))
                })
            }
